...
voom_fit <- voom(y, design, plot = FALSE)
fit <- lmFit(voom_fit, design)
fit <- eBayes(fit, robust = TRUE)

res_df <- as.data.frame(topTable(fit, coef = 2, number = Inf, sort.by = "none"))
res_df <- cbind(gene_id = rownames(res_df), res_df)
if ("adj.P.Val" %in% names(res_df)) {
  res_df$significant <- res_df$adj.P.Val <= fdr_threshold
  res_df <- res_df[order(res_df$adj.P.Val, res_df$P.Value), , drop = FALSE]
}
